	...
	return this.each(function()
	{
		...
		function refresh_simple_pagination(page_number)
		{
			...
			if(settings.use_page_x_of_x)
			{
				...
				$(container_id + ' .' + settings.html_prefix + '-page-x-sur-x').html(page_x_of_x_html);
			}
			...
			{
				var showing_x_of_x_html = settings.showing_x_of_x_content + ' ' + simple_number_formatter(item_range_min + 1, 0, settings.thousands_separator) + '-' + simple_number_formatter(item_range_max, 0, settings.thousands_separator) + ' sur ' + simple_number_formatter(item_count, 0, settings.thousands_separator);
...
$.fn.simplePagination.defaults = {
	...
	//
	first_content: 'Première page',  //e.g. '<<'
	previous_content: 'Précédent',  //e.g. '<'
	next_content: 'Suivant',  //e.g. '>'
	last_content: 'Dernière page', //e.g. '>>'
	page_x_of_x_content: 'Page',
	showing_x_of_x_content: 'Afficher',
	//
